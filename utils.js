...
async function getRawFileContent(rawFileUrl, maxRetries = GET_FILE_MAX_RETRIES) {
    let retries = 0;
    ...
    throw new Error(`Max retries (${maxRetries}) exceeded. Unable to fetch the file.`);
}
...
async function screenshotUrl(url) {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            throw new Error(`Failed to fetch the screenshot. Status code: ${response.status}`);
        }
    } catch (error) {
